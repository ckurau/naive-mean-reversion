# backtest_nmr_lib_v35.py
# V35 -- V34 baseline + streak filter Option C
#
# STREAK FILTER DATA (from trade outcome analysis on V34 trades):
#   Baseline WR: 58.6%
#   After 3 consecutive losses: WR = 26.6% (n=3,423) -- edge has broken down
#   After 5 consecutive losses: WR = 20.9% (n=1,930) -- deeply negative EV
#   Streak trades (45.8% of all trades): -$13.98M total P&L
#   Non-streak trades (54.2%):           +$14.65M total P&L
#   Streaks occur 912 times, avg length 5.8, max 47 consecutive losses
#
# OPTION C IMPLEMENTATION:
#   After 3 consecutive losses: reduce position size to 50% of normal
#   After 5 consecutive losses: pause all new entries for 3 calendar days
#   Reset on first winning trade (streak counter back to 0, full size restored)
#   After pause expires: resume at 50% size until a win resets fully
#
# SIZE INTERACTION WITH C5:
#   During half-size streak: C5 boost is suppressed (no boosting into losses)
#   Outside streak: C5 boost applies normally to top 20% signals
#
# V34 CHANGES CARRIED FORWARD (permanent):
#   [C3] GAP_DOWN_MAX = -0.010
#   [C5] Top 20% of signals get 1.2x size, hard cap 12%
#
# EXPERIMENTS:
#   V34_baseline  -- V34 unchanged (control)
#   V35_streak_c  -- V34 + Option C streak filter
...
class ExperimentConfig:
    ...
    description: str = "V34 -- no streak filter"
    use_streak_filter: bool = False
    streak_half_size_threshold: int = 3
    streak_pause_threshold: int = 5
    streak_pause_days: int = 3
    streak_half_size_multiplier: float = 0.50


BASELINE = ExperimentConfig(
    name="V34_baseline",
    description="V34 -- no streak filter (control)",
    use_streak_filter=False,
)

V35 = ExperimentConfig(
    name="V35_streak_c",
    description="V35 -- 3 losses=50% size, 5 losses=pause 3d, reset on win",
    use_streak_filter=True,
    streak_half_size_threshold=3,
    streak_pause_threshold=5,
    streak_pause_days=3,
    streak_half_size_multiplier=0.50,
)
...
# -----------------------------------------------------------------------------
# Config -- V34 permanent parameters
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 6. Streak state tracker
# -----------------------------------------------------------------------------
class StreakTracker:
    def __init__(self, cfg: ExperimentConfig):
        self.cfg = cfg
        self.consecutive_losses = 0
        self.pause_until = None   # date object or None

    def record_trade(self, pnl_usd: float, exit_date):
        if not self.cfg.use_streak_filter:
            return
        if pnl_usd > 0:
            self.consecutive_losses = 0
            ...
            if self.consecutive_losses >= self.cfg.streak_pause_threshold:
                pause_end = (pd.Timestamp(exit_date) +
                             pd.Timedelta(days=self.cfg.streak_pause_days)).date()
                if self.pause_until is None or pause_end > self.pause_until:
                    self.pause_until = pause_end

    def is_paused_on(self, today) -> bool:
        if not self.cfg.use_streak_filter:
            return False
        if self.pause_until is None:
            return False
        return pd.Timestamp(today).date() <= self.pause_until

    @property
    def size_multiplier(self) -> float:
        if not self.cfg.use_streak_filter:
            return 1.0
        if self.consecutive_losses >= self.cfg.streak_half_size_threshold:
            return self.cfg.streak_half_size_multiplier
        return 1.0
...
def run_backtest(price_data, spy_df, vix_df, sector_data, earnings_map,
    ...
    streak = StreakTracker(cfg)
    diag_half_size = 0
    diag_paused_days = 0

    for today in tqdm(trading_dates, desc=f"Simulate [{cfg.name}]"):
        spy_ok = spy_regime.get(today, True)
        paused_vix, last_vix_spike = check_vix_spike(today, vix_df, last_vix_spike)
        ...
        streak_paused = streak.is_paused_on(today)
        if streak_paused:
            diag_paused_days += 1
        ...
        if not spy_ok or paused_vix or velocity_paused or streak_paused:
            continue
        ...
        for rank, (composite_score, tkr, consec_val, rsi_val) in enumerate(candidates):
            ...
            # C5 boost only when not in a streak -- suppress boosting into losses
            if streak_mult == 1.0 and n_candidates >= MIN_CANDIDATES_FOR_C5 and rank < top_n:
                combined_mult = TOP_SIGNAL_MULTIPLIER
            else:
    ...
    if cfg.use_streak_filter:
        print(f"[Backtest] Streak diagnostics: half-size entries={diag_half_size}, "
              f"paused trading days={diag_paused_days}")
...
def compute_metrics(trades_df: pd.DataFrame, cfg: ExperimentConfig = BASELINE) -> tuple:
    ...
    metrics = {
        ...
        "streak_filter_active": cfg.use_streak_filter,
        "streak_params": {
            "half_size_at":   cfg.streak_half_size_threshold,
            "pause_at":       cfg.streak_pause_threshold,
            "pause_days":     cfg.streak_pause_days,
            "half_size_mult": cfg.streak_half_size_multiplier,
        } if cfg.use_streak_filter else {},
...
__all__ = [
    "ExperimentConfig", "BASELINE", "V35", "EXPERIMENTS",
