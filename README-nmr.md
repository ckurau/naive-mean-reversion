...
Note: V34 OOS avg CAGR (19.45%) improved vs V33d (18.37%). W3 2013-2014 jumped from 34.5% to 39.6% -- the C3 gap filter removed adverse fills that were hurting OOS performance. W7 2021-2022 remains the only failure window. W8 2023-2025 at 5.1% is positive but the caution window.
...
## Walk-Forward Reference (V33d -- superseded by V34 above)

### V33d Walk-Forward (previous version, for reference)
...
OOS Positive CAGR windows: 7/8 | OOS Avg CAGR: 18.37% | OOS Median CAGR: 19.38%

### V32e Walk-Forward (for reference)
...
OOS Positive CAGR windows: 6/8 | OOS Avg CAGR: 5.58%

---

## Key Insights

### The strategy is fully optimised -- V34 is the confirmed ceiling

After 50+ versions tested across multiple experiment batches, every meaningful lever has been exhausted. The consistent finding is:

**The strategy must be fully exposed during panic to capture recovery. Any mechanism that reduces this exposure costs more in recovery years than it saves in bad years.**

This is structural, not a calibration problem. The days with the worst avg return (high signal count, crash days) are simultaneously the days that set up the best recovery compounding. Every filter that avoids bad days also blocks the recovery that follows.
...
The 2%/8d structure is confirmed optimal. The avg loss of -3.61% is structural and cannot be reduced without destroying win rate.

### The entry filter side is fully saturated

Seven structural entry filters all failed including the V38a batch:
- **V36a** (signal density): -$519k
- **V37a** (breadth filter): -$2,899k, fired ~50% of days
- **V37b** (divergence): -$455k, fired only 1.2% of days
- **V37c** (MFE pause): miscalibrated, fired 54.8% of days
- **V37d** (Friday filter): -$1,340k
- **V38a-C1** (IBS < 0.35): -$2,150k, killed 26% of trades (crash-recovery days)
- **V38a-C2** (EMA 20/50 stack): -$1,470k, worsened MaxDD simultaneously

**Do not retry structural entry filters.**

### What the V38a/V39a experiment sessions proved

Five changes were tested in V38a (C1 through C5). Only two survived:
- **C3** (gap filter -1.0%): +$324k standalone, confirmed positive
- **C5** (tiered sizing): +$267k standalone, confirmed positive on C3 base

Three were rejected: C1 IBS blocked crash-recovery trades (-$2.15M), C2 EMA stack did the same (-$1.47M), C4 double cooldown was neutral (fires too rarely).

The combined V38a_all experiment confirmed that C1 and C2 amplify each other's damage when combined (-$2.5M from baseline). Never combine filters that reduce crash-day exposure.
...
| **Signal density stress filter (V36a)** | >40 signals -> 0.5x size | -$519k. Crash recovery days both worst avg return AND best absolute P&L |
...
| **IBS < 0.35 filter (V38a-C1)** | (Close-Low)/(High-Low) < 0.35 | -$2,150k. Crash-recovery days close mid-range intraday but still snap back |
| **EMA 20/50 downtrend block (V38a-C2)** | Skip entries when Close < EMA20 < EMA50 | -$1,470k. Blocked crash-recovery entries AND worsened MaxDD |
| **Double time-stop cooldown (V38a-C4)** | 15d cooldown if stock time-stops twice in 30d | Neutral -- fires too rarely to matter |
...
| Metric | SPY B&H | Run 5 | V32e | V33d | V34 | V32d (Roth) |
|---|---|---|---|---|---|---|
| CAGR (gross, in-sample) | ~10.5% | 7.58% | 16.10% | 17.41% | 18.50% | 15.37% |
| CAGR (gross, OOS est.) | ~10.5% | 5.58% | ~12% | ~13% | ~14% | TBD |
| CAGR (after tax, est.) | ~8.4% | ~3.5-5% | ~6-9% | ~7-10% | ~8-10% | tax-free (Roth) |
| Max Drawdown | -55% (2008) | -22.55% | -48.61% | -54.73% | -54.50% | -39.21% |
| Sharpe Ratio | ~0.55 | 0.73 | 0.73 | 0.68 | 0.71 | 0.77 |
| Final Equity ($100k start) | ~$800k | $478k | $2,454k | $3,124k | $3,806k | $2,145k |
...
- **MAX_POSITIONS in trade.py:** Should be 60 to match V34

### IMPORTANT: trade.py runs locally, not from GitHub

`trade.py` runs from `C:\nmr-trader\` on your local PC. **It does NOT pull updated files from GitHub automatically.** The backtest files (`backtest_nmr_lib.py`) that live in GitHub are separate from what trade.py uses.

**What this means for V34:** The two V34 changes in `backtest_nmr_lib.py` do NOT automatically apply to paper trading. To sync paper trading with V34 exactly, you need to manually update `trade.py` on your local machine:

1. `GAP_DOWN_MAX = -0.010` -- change from -0.015 if present (this is the C3 change)
2. Tiered sizing logic -- top 20% of candidates get 1.2x size multiplier (C5 change)

For paper trading validation purposes, these differences are minor (~18 trades/year affected by C3, C5 affects sizing only). The core strategy edge being validated is the same. But if you want exact V34 behaviour in paper trading before going live, update trade.py to match.

**To check your current trade.py settings:** open `C:\nmr-trader\trade.py` and search for `GAP_DOWN_MAX` and any position sizing logic near the entry section.
...
| 0 entry candidates | SPY below 200d MA | Normal in bear market -- 2022, parts of 2025-2026 |
...
V34: OOS avg 19.45%, 7/8 positive. V33d: OOS avg 18.37%, 7/8 positive. V32e: OOS avg 5.58%, 6/8 positive.
...
The strategy is fully optimised through V34. Do not retry entry filters or exit mechanics. Remaining genuine improvements:
- **Historical earnings database** -- removes lookahead bias from yfinance forward-looking calendar. Est. +0.3 to +0.5% CAGR
...
- **trade.py V34 sync** -- update trade.py GAP_DOWN_MAX and tiered sizing to match V34 exactly before going live

---

## All Runs Table

### V21-V34: Complete Sequence
...
| V36a | Signal stress: >40 signals -> 0.5x size | 16.42% | $2,605k | -52.19% | 0.67 |
...
*V37c ran only to 2014 due to miscalibration. Numbers not comparable.
