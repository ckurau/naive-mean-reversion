...
| **Gap filter further tightening -- -0.75% (V40a)** | GAP_DOWN_MAX = -0.0075 | CAGR -2.59pp, equity -$1,433k, MaxDD worse -3.72pp. Gap ceiling confirmed at -1.0%. |
| **Gap filter further tightening -- -0.50% (V40b)** | GAP_DOWN_MAX = -0.005 | CAGR -4.53pp, equity -$2,154k, MaxDD worse -4.30pp. Strongly negative. |
...
- **MAX_POSITIONS in trade.py:** 60 (matches V35)
...
### trade.py V35 sync checklist

| Change | Parameter | V34 value | V35 value | Status |
|---|---|---|---|---|
...
| **C6 tier multiplier** | **TOP_SIGNAL_MULTIPLIER** | **1.20** | **1.30** | **Apply now** |
...
**To verify trade.py is current after V35 update:** open `C:\nmr-trader\trade.py` and confirm:
- Line: `GAP_DOWN_MAX = -0.010`
- Line: `TOP_SIGNAL_MULTIPLIER = 1.30` (or equivalent constant/inline value)
...
| Win rate < 55% after 50 trades | Signal logic drift | Verify trade.py constants match V35 |
