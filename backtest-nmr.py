""" Enhanced Naive Mean Reversion (MR) Backtest — V32e
==================================================
Base: V30+S&P600 — $2,414,283 final equity, 16.01% CAGR.

V32e CHANGE (1 addition):
  [V32e] Composite entry ranking: sort by RSI(2)/ATR_pct instead of RSI(2) alone.
  Prioritizes candidates that are both deeply oversold AND experiencing
  high volatility — the strongest mean reversion setups.
  On constrained days (more signals than open slots), better trades
  are selected. Trade volume unchanged.

V32e RESULT: $2,454,235 | CAGR 16.10% | Sharpe 0.73 | MaxDD -48.61%
  vs V30+S&P600: +$40k equity, +0.09% CAGR, no other changes.

RESULTS HISTORY:
  Run 5:       CAGR  7.58% | $478k
  V30:         CAGR 14.42% | $1,797k  (S&P 500+400)
  V30+S&P600:  CAGR 16.01% | $2,414k
  V32e:        CAGR 16.10% | $2,454k  ← current best (taxable account)
...
def save_outputs(trades_df, metrics, eq_df):
    ...
    print("\n" + "=" * 70)
    print("  NAIVE MR BACKTEST — V32e (S&P 500 + 400 + 600)")
    print("=" * 70)
