# V36 TEST BUILD -- backtest_nmr_lib_v36_tests.py
#
# Adds TWO isolated, independently togglable tests on top of confirmed V35 baseline.
#
# TEST 1 -- Rolling win rate adaptive sizing (targets W7/W8 slow-bear failure)
#   ENABLE_TEST1_ROLLING_WR = True
#   Logic: track last N completed trades (excl. partial_exit).
#          If rolling win rate < WR_THRESHOLD, apply WR_SIZE_MULTIPLIER to all new entries.
#          Reacts to LIVE TRADE OUTCOMES, not price filters -- does NOT block crash-recovery entries.
#   Parameters added: 2 (WR_LOOKBACK, WR_THRESHOLD) + 1 multiplier = low overfit risk.
#
# TEST 2 -- RSI(2) threshold tightening in low-VIX environment (targets W5 low-vol drought)
#   ENABLE_TEST2_VIX_RSI = True
#   Logic: when VIX < VIX_LOW_REGIME, require RSI(2) < RSI_THRESHOLD_TIGHT (default 15 not 20).
#          Only fires in genuinely low-vol regimes. No effect on high-VIX recovery windows.
#   Parameters added: 2 (VIX_LOW_REGIME, RSI_THRESHOLD_TIGHT) = low overfit risk.
#
# HOW TO RUN EACH TEST:
#   Baseline only (V35):  ENABLE_TEST1_ROLLING_WR = False, ENABLE_TEST2_VIX_RSI = False
#   Test 1 only:          ENABLE_TEST1_ROLLING_WR = True,  ENABLE_TEST2_VIX_RSI = False
#   Test 2 only:          ENABLE_TEST1_ROLLING_WR = False, ENABLE_TEST2_VIX_RSI = True
#   Both combined:        ENABLE_TEST1_ROLLING_WR = True,  ENABLE_TEST2_VIX_RSI = True
#
# WHAT TO LOOK FOR in results vs V35 baseline:
#   Test 1: MaxDD improvement in W7 (2021-22) OOS window. CAGR should hold within -1pp.
#            If W6 (2019-20) CAGR drops by more than 2pp, the rolling WR is firing too early
#            in crash-recovery and needs WR_LOOKBACK increased or WR_THRESHOLD lowered.
#   Test 2: W5 (2017-18) Profit Factor improvement from 1.05 toward 1.10+.
#            Trade count should fall ~10-15% in low-VIX years (2017, 2013-14 bull).
#            If overall CAGR drops > 1pp, the threshold is too tight -- raise VIX_LOW_REGIME.
#
# DO NOT RETRY (from V35 research -- these are confirmed dead ends):
#   - Price-based stop losses, circuit breakers, VIX spike exits
#   - Streak filters (destroys crash-recovery entries)
#   - Friday filters, breadth filters, IBS filters
#   - Any filter that responds to price rather than trade outcomes
#
# V35 confirmed baseline:
#   CAGR: 18.81% | Final Equity: $4,028,420 | MaxDD: -56.22% | Sharpe: 0.71
# =============================================================================
...
ENABLE_TEST2_VIX_RSI    = True   # RSI threshold tightening in low-VIX

# =============================================================================
# TEST 1 PARAMETERS -- Rolling win rate adaptive sizing
# =============================================================================
WR_LOOKBACK      = 20    # Number of completed trades to track (excl. partial_exit)
WR_THRESHOLD     = 0.55  # If rolling WR falls below this, apply size reduction
WR_SIZE_MULT     = 0.50  # Multiply base position size by this when WR < threshold
                          # 0.50 = half size. Do not go below 0.25 (too aggressive).

# =============================================================================
# TEST 2 PARAMETERS -- RSI threshold tightening in low-VIX
# =============================================================================
VIX_LOW_REGIME        = 15   # VIX below this = low-volatility regime
RSI_THRESHOLD_TIGHT   = 15   # Use this RSI(2) threshold instead of 20 when VIX < VIX_LOW_REGIME

# =============================================================================
# V35 Baseline config (unchanged)
# =============================================================================
...
RSI_PERIOD         = 2
RSI_THRESHOLD      = 20   # Default; may be overridden per-trade by Test 2
ATR_PERIOD         = 14
ATR_MIN_PCT        = 0.01
VOL_MA_PERIOD      = 20
MIN_HOLD_BEFORE_EXIT = 2

# Tier system -- uniform 2%/8d (V35 confirmed)
TIER1_MIN_DOWN      = 6
TIER1_TARGET        = 0.020
TIER1_HOLD_DAYS     = 8
TIER1_PARTIAL       = True
...
# V35 tiered signal sizing
TOP_SIGNAL_PCT        = 0.20
TOP_SIGNAL_MULTIPLIER = 1.30   # V35 confirmed
...
def generate_signals(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Base signal -- RSI threshold is RSI_THRESHOLD (20) here.
    # Test 2 further filters at the entry loop using real-time VIX value.
    df["signal"] = (
        df["above_ma"]
...
# -----------------------------------------------------------------------------
# TEST 1 HELPER: rolling win rate tracker
# -----------------------------------------------------------------------------
class RollingWinRate:
    """
    Tracks last WR_LOOKBACK completed trades (excluding partial_exit).
    Returns the current size multiplier to apply to new entries.
    Logic: if rolling WR < WR_THRESHOLD → apply WR_SIZE_MULT (e.g. 0.50).
           Otherwise → 1.0 (no change).
    Only activates once at least WR_LOOKBACK trades have been completed
    to avoid firing during startup on too little data.
    """
    def __init__(self):
        self.outcomes: deque = deque(maxlen=WR_LOOKBACK)

    def record(self, pnl_usd: float):
        """Call this each time a non-partial trade completes."""
        self.outcomes.append(1 if pnl_usd > 0 else 0)

    def size_multiplier(self) -> float:
        if not ENABLE_TEST1_ROLLING_WR:
            return 1.0
        if len(self.outcomes) < WR_LOOKBACK:
            return 1.0  # Not enough history yet -- don't penalize startup
        wr = sum(self.outcomes) / len(self.outcomes)
        return WR_SIZE_MULT if wr < WR_THRESHOLD else 1.0

    def current_wr(self) -> float | None:
        if len(self.outcomes) < WR_LOOKBACK:
            return None
...
def run_backtest(price_data, spy_df, vix_df, sector_data, earnings_map) -> pd.DataFrame:
    tests_active = []
    if ENABLE_TEST1_ROLLING_WR:
        tests_active.append(f"TEST1:RollingWR(lookback={WR_LOOKBACK},threshold={WR_THRESHOLD},mult={WR_SIZE_MULT})")
    if ENABLE_TEST2_VIX_RSI:
        tests_active.append(f"TEST2:VIX-RSI(vix<{VIX_LOW_REGIME}->RSI<{RSI_THRESHOLD_TIGHT})")
    test_label = " | ".join(tests_active) if tests_active else "BASELINE V35 (no tests active)"

    print(f"\n[Backtest] Running simulation ...")
    print(f"[Backtest] {test_label}")
    print(f"[Backtest] GAP_DOWN_MAX={GAP_DOWN_MAX} | TOP_SIGNAL_MULTIPLIER={TOP_SIGNAL_MULTIPLIER}")
    ...
    # Test 1 state
    rolling_wr = RollingWinRate()

    # Diagnostic counters
    t1_reductions = 0   # Times Test 1 triggered a size reduction
    t2_filtered   = 0   # Trades filtered by Test 2
    ...
    for today in tqdm(trading_dates, desc="Simulating"):
        ...
        # --- Get real-time VIX value for Test 2 ---
        current_vix = None
        if ENABLE_TEST2_VIX_RSI:
            try:
                vc = vix_df["Close"].squeeze()
                if today in vc.index:
                    current_vix = float(vc.loc[today])
            except Exception:
                pass
        ...
        for tkr, pos in open_positions.items():
            ...
            if (pos["partial_enabled"] and not pos["partial_done"]
                ...
                trades.append({
                    ...
                    "test1_active": pos.get("test1_active", False),
                    "test2_filtered": False,
                })
                portfolio_value         += pnl
            ...
            if full_exit:
                ...
                # Test 1: record outcome for rolling WR (exclude partial_exit)
                rolling_wr.record(pnl)
        ...
        for tkr, tkr_df in signals.items():
            ...
            # -----------------------------------------------------------------
            # TEST 2: RSI threshold tightening in low-VIX environments
            # Applies BEFORE adding to candidates -- genuinely reduces signal set.
            # Only fires when VIX < VIX_LOW_REGIME. High-VIX windows unaffected.
            # -----------------------------------------------------------------
            if ENABLE_TEST2_VIX_RSI and current_vix is not None:
                if current_vix < VIX_LOW_REGIME and rsi2 >= RSI_THRESHOLD_TIGHT:
                    t2_filtered += 1
                    continue  # Skip this signal -- not oversold enough in low-vol
        ...
        # -----------------------------------------------------------------
        # TEST 1: get current rolling WR multiplier (applies to all entries this day)
        # -----------------------------------------------------------------
        wr_mult = rolling_wr.size_multiplier()
        if ENABLE_TEST1_ROLLING_WR and wr_mult < 1.0:
            t1_reductions += 1
        ...
        for rank, (composite_score, tkr, consec_val, rsi_val) in enumerate(candidates):
            ...
            # TEST 1: apply rolling WR multiplier on top of V35 sizing
            # Note: applies AFTER the top-signal multiplier, so it scales the
            # already-boosted size down proportionally. Hard cap still applies.
            combined_multiplier = size_multiplier * wr_mult
            ...
            open_positions[tkr] = {
                ...
                "test1_active":         (ENABLE_TEST1_ROLLING_WR and wr_mult < 1.0),
            }

    print(f"\n[Backtest] Complete -- {len(trades)} trades executed.")
    if ENABLE_TEST1_ROLLING_WR:
        print(f"[Test 1]  Rolling WR size reductions triggered on {t1_reductions} trading days.")
    if ENABLE_TEST2_VIX_RSI:
        print(f"[Test 2]  Signals filtered by VIX-RSI rule: {t2_filtered}")
...
def compute_metrics(trades_df: pd.DataFrame) -> tuple:
    ...
    # Test 1 diagnostic: how often was size reduced and what was the outcome?
    test1_stats = {}
    if ENABLE_TEST1_ROLLING_WR and "test1_active" in trades_df.columns:
        t1_on  = trades_df[trades_df["test1_active"] == True]
        t1_off = trades_df[trades_df["test1_active"] == False]
        test1_stats = {
            "trades_at_reduced_size":   len(t1_on),
            "win_rate_reduced":         round((t1_on["pnl_usd"] > 0).mean() * 100, 1) if len(t1_on) else 0,
            "pnl_reduced":              round(t1_on["pnl_usd"].sum(), 2) if len(t1_on) else 0,
            "trades_at_full_size":      len(t1_off),
            "win_rate_full":            round((t1_off["pnl_usd"] > 0).mean() * 100, 1) if len(t1_off) else 0,
            "pnl_full_size":            round(t1_off["pnl_usd"].sum(), 2) if len(t1_off) else 0,
        }
    ...
    tests_active_str = []
    if ENABLE_TEST1_ROLLING_WR:
        tests_active_str.append(f"T1:RollingWR(lb={WR_LOOKBACK},thr={WR_THRESHOLD},mult={WR_SIZE_MULT})")
    if ENABLE_TEST2_VIX_RSI:
        tests_active_str.append(f"T2:VIX-RSI(vix<{VIX_LOW_REGIME}->RSI<{RSI_THRESHOLD_TIGHT})")
    version_label = "V36-" + "+".join(tests_active_str) if tests_active_str else "V35-BASELINE"

    metrics = {
        "version":              version_label,
        "tests_active": {
            "test1_rolling_wr": ENABLE_TEST1_ROLLING_WR,
            "test2_vix_rsi":    ENABLE_TEST2_VIX_RSI,
        },
        ...
        "parameters": {
            ...
            "test1_enabled":          ENABLE_TEST1_ROLLING_WR,
            "test1_wr_lookback":      WR_LOOKBACK,
            "test1_wr_threshold":     WR_THRESHOLD,
            "test1_wr_size_mult":     WR_SIZE_MULT,
            "test2_enabled":          ENABLE_TEST2_VIX_RSI,
            "test2_vix_low_regime":   VIX_LOW_REGIME,
            "test2_rsi_tight":        RSI_THRESHOLD_TIGHT,
...
def save_outputs(trades_df, metrics, eq_df):
    ...
    print("\n" + "=" * 70)
    print(f" NMR BACKTEST -- {metrics.get('version','V36')}")
    print("=" * 70)
    ...
    if metrics.get("test1_diagnostic"):
        print(f"\n  Test 1 Diagnostic (rolling WR):")
        for k, v in metrics["test1_diagnostic"].items():
            print(f"    {k:<36}: {v}")
...
__all__ = [
    ...
    "ENABLE_TEST1_ROLLING_WR", "ENABLE_TEST2_VIX_RSI",
]
