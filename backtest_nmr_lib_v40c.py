...
EARNINGS_BLACKOUT = 3
GAP_DOWN_MAX = -0.010    # [V40c] same as V34 tightened from -0.010
GAP_UP_MAX = 0.020
SECTOR_MA_WINDOW = 20
...
TOP_SIGNAL_PCT = 0.20
TOP_SIGNAL_MULTIPLIER = 1.30   # [V40c] increased from 1.20
TOP_SIGNAL_HARD_CAP = 0.12
MIN_CANDIDATES_FOR_C5 = 5

VERSION_TAG = "V40c"
OUTPUT_DIR = Path("results/v40/v40c")
...
def run_backtest(price_data, spy_df, vix_df, sector_data, earnings_map) -> pd.DataFrame:
    ...
    print(f"[Backtest] GAP_DOWN_MAX={GAP_DOWN_MAX} [V40c same as V34] | "
    ...
    for today in tqdm(trading_dates, desc="Simulating"):
        ...
        for rank, (composite_score, tkr, consec_val, rsi_val) in enumerate(candidates):
            ...
            # [V40c] Gap filter unchanged from V34
...
def compute_metrics(trades_df: pd.DataFrame) -> tuple:
    ...
    metrics = {
        "version": VERSION_TAG,
        "test_description": "V34 base + TOP_SIGNAL_MULTIPLIER increased 1.20x -> 1.30x",
        "vs_v34_changes": {"TOP_SIGNAL_MULTIPLIER": "1.30 (was 1.20)"},
