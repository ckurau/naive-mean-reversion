# backtest_nmr_lib_v47.py
#
# V47 = V35 + I_Combo_BCDE (confirmed best result from Ideas V5 session)
#
# Changes vs V35 (backtest_nmr_lib.py):
#   B_TOM_SIZING    Last trading day of month + next 3 days: size x1.15
#   C_VIX_RSI       When VIX < 15: require RSI(2) < 15 instead of < 20
#   D_PARTIAL_TUNE  Tier 1 partial trigger 1.0% -> 0.8%
#   E_DOW_SIZING    Tuesday x1.10, Friday x0.90 (RenTec day-of-week seasonality)
#
# Ideas V5 results vs V35+I3 baseline (21.72% CAGR, $8.78M):
#   I_Combo_BCDE: +0.68pp CAGR, +$1.14M equity, -0.88pp MaxDD
#
# Architecture: drop-in replacement for backtest_nmr_lib.py.
# walkforward_v47.py imports run_backtest from this module.
# All V35 logic is preserved; only the four overrides above are added.
...
TIER1_PARTIAL_TRIGGER = 0.008   # V47 change D: was 0.010
...
# ---------------------------------------------------------------------------
# V47-specific parameters
# ---------------------------------------------------------------------------

# B: Turn-of-month sizing
TOM_MULT = 1.15        # extra multiplier during TOM window

# C: VIX low-regime RSI tightening
VIX_TIGHT_THRESH = 15.0   # VIX below this -> tighter RSI
RSI_TIGHT_THRESH = 15.0   # RSI threshold in low-VIX regime (was 20)

# E: Day-of-week sizing
DOW_MULT = {
    0: 1.00,   # Monday
    1: 1.10,   # Tuesday — reversal bias
    2: 1.00,   # Wednesday
    3: 1.00,   # Thursday
    4: 0.90,   # Friday — pre-weekend position closing
}
...
def get_tier(consec_down: int) -> dict:
    if consec_down >= TIER1_MIN_DOWN:
        ...
        return {"tier": 2, "profit_target": TIER2_TARGET, "hold_days": TIER2_HOLD_DAYS,
                "partial_enabled": False, "partial_frac": 0.0,
...
# ---------------------------------------------------------------------------
# 4. Signal generation (unchanged from V35)
# ---------------------------------------------------------------------------
def generate_signals(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Signal uses RSI_THRESHOLD=20; C override applied at runtime per-day VIX
    df["signal"] = (
        df["above_ma"]
...
# ---------------------------------------------------------------------------
# 6. Backtest simulation — V35 loop + V47 overrides
# ---------------------------------------------------------------------------
def run_backtest(price_data, spy_df, vix_df, sector_data, earnings_map) -> pd.DataFrame:
    print(f"\n[Backtest] Running {VERSION_TAG} ...")
    print(f"  Changes vs V35: TOM x{TOM_MULT} | VIX<{VIX_TIGHT_THRESH}->RSI<{RSI_TIGHT_THRESH} "
          f"| Partial trigger {TIER1_PARTIAL_TRIGGER:.3f} | DOW mult Tue/Fri {DOW_MULT[1]}/{DOW_MULT[4]}")
    ...
    for today in tqdm(trading_dates, desc="Simulating"):
        ...
        # C: tighten RSI on low-VIX days
        rsi_thresh = RSI_TIGHT_THRESH if vix_now < VIX_TIGHT_THRESH else RSI_THRESHOLD
        ...
        for tkr, tkr_df in signals.items():
            ...
            if not row["signal"]:
                continue
            # C: extra RSI gate on low-VIX days
            if vix_now < VIX_TIGHT_THRESH and float(row["rsi2"]) >= rsi_thresh:
                continue
        ...
        for rank, (score, tkr, consec_val, rsi_val) in enumerate(candidates):
            ...
            tier_cfg = get_tier(consec_val)  # D already baked into TIER1_PARTIAL_TRIGGER
...
def compute_metrics(trades_df: pd.DataFrame) -> tuple:
    ...
    metrics = {
        "version":         VERSION_TAG,
        "description":     "V35 + TOM sizing x1.15 + VIX<15 RSI<15 + Partial trigger 0.8% + DOW Tue/Fri",
        "vs_v35_changes":  {
            "TOM_MULT":            TOM_MULT,
            "VIX_TIGHT_THRESH":    VIX_TIGHT_THRESH,
            "RSI_TIGHT_THRESH":    RSI_TIGHT_THRESH,
            "TIER1_PARTIAL_TRIGGER": TIER1_PARTIAL_TRIGGER,
            "DOW_MULT_TUE":        DOW_MULT[1],
            "DOW_MULT_FRI":        DOW_MULT[4],
        },
...
def save_outputs(trades_df, metrics, eq_df):
    ...
    print(f"\n{'='*60}")
    print(f" {VERSION_TAG}: {metrics.get('description','')}")
    print(f"{'='*60}")
    ...
    print(f"  {'vs_v35_changes':<28}: {metrics.get('vs_v35_changes','')}")
    print(f"{'='*60}")
...
__all__ = [
    ...
    "MAX_POSITIONS", "MA_WINDOW", "VOL_MA_PERIOD", "ATR_PERIOD",
    "RSI_PERIOD", "RSI_THRESHOLD", "ATR_MIN_PCT", "MIN_DOLLAR_VOLUME",
    "MIN_CONSEC_DOWN", "MIN_HOLD_BEFORE_EXIT",
    "VELOCITY_CRASH_5D_THRESHOLD", "VELOCITY_CRASH_PAUSE_DAYS",
    "VIX_SPIKE_PAUSE_DAYS", "VIX_LOW", "GAP_DOWN_MAX", "GAP_UP_MAX",
    "REENTRY_COOLDOWN_DAYS", "MAX_SECTOR_POSITIONS",
    "EARNINGS_BLACKOUT", "EARNINGS_MONTHS",
