...
EARNINGS_BLACKOUT = 3
GAP_DOWN_MAX = -0.005    # [V40b] tightened from -0.010
GAP_UP_MAX = 0.020
SECTOR_MA_WINDOW = 20
...
VERSION_TAG = "V40b"
OUTPUT_DIR = Path("results/v40/v40b")
...
def run_backtest(price_data, spy_df, vix_df, sector_data, earnings_map) -> pd.DataFrame:
    ...
    print(f"[Backtest] GAP_DOWN_MAX={GAP_DOWN_MAX} [V40b tightened] | "
...
def compute_metrics(trades_df: pd.DataFrame) -> tuple:
    ...
    metrics = {
        "version": VERSION_TAG,
        "test_description": "V34 base + GAP_DOWN_MAX tightened -1.0% -> -0.50%",
        "vs_v34_changes": {"GAP_DOWN_MAX": "-0.005 (was -0.010)"},
